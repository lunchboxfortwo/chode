...
class WhaleBot(BaseBot):
    ...
    def decide_preflop(self, position, stack, pot, to_call, facing_raise, raise_position, last_raise, bb,
        ...
        if not will_play:
            return Action("fold", strategy_note="whale/fold")
        ...
        if facing_raise:
            if raise_roll < 0.55:
                ...
                return Action("raise", amt, strategy_note="whale/reraise")
            return Action("call", min(to_call, stack), strategy_note="whale/call")
        else:
            if raise_roll < 0.70:
                amt = min(bb * 4, stack)
                return Action("raise", amt, strategy_note="whale/open")
            return Action("call", min(bb, stack), strategy_note="whale/limp")
    ...
    def decide_postflop(self, board, position, stack, pot, to_call, is_first_to_act, action_sequence=None, prev_street_actions=None, n_active=2, postflop_player_idx=0) -> Action:
        ...
        if to_call > 0:
            if top_rank >= 10 or bluff:
                if random.random() < 0.40 and to_call < pot * 0.5:
                    ...
                    return Action("raise", raise_amt, strategy_note="whale/aggro")
                return Action("call", min(to_call, stack), strategy_note="whale/call")
            return Action("fold", strategy_note="whale/fold")
        ...
        if bluff or top_rank >= 11:
            ...
            return Action("bet", min(bet_size, stack), strategy_note="whale/bet")
        return Action("check", strategy_note="whale/check")
